...
namespace AgriSaarthi.Api.Services
{
    public class WeatherService : IWeatherService
    {
        ...
        public WeatherService(AgriSaarthiDbContext context, HttpClient httpClient, ILogger<WeatherService> logger)
        {
            _context = context;
        ...
        public async Task<WeatherResponseDto?> GetWeatherForFarmerAsync(int farmerId)
        {
            ...
            if (locationParts.Count == 0) return null; 

            try
            {
                // 1. Geocoding with Fallback
                OpenMeteoLocation? location = null;
                
                // Try combinations: Full, then without Location (village), then just State
                var searchQueries = new List<string>();
                searchQueries.Add(string.Join(", ", locationParts));
                
                if (locationParts.Count >= 2 && !string.IsNullOrWhiteSpace(profile.District))
                {
                    var fallbackParts = new List<string> { profile.District };
                    if (!string.IsNullOrWhiteSpace(profile.State)) fallbackParts.Add(profile.State);
                    searchQueries.Add(string.Join(", ", fallbackParts));
                }
                
                if (locationParts.Count >= 3 && !string.IsNullOrWhiteSpace(profile.State))
                {
                    searchQueries.Add(profile.State);
                }

                foreach (var query in searchQueries)
                {
                    string geoUrl = $"https://geocoding-api.open-meteo.com/v1/search?name={Uri.EscapeDataString(query)}&count=1";
            ...
            {
                _logger.LogError(ex, "Weather service HTTP request failed for farmer {FarmerId}", farmerId);
                throw new ApplicationException("EXTERNAL_API_ERROR", ex);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Weather service encountered an unexpected error for farmer {FarmerId}", farmerId);
                throw;
            }
        }
